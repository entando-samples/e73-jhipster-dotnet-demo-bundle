...
```bash
dotnet run --verbosity normal --project ./src/JhipsterDotNetMS/JhipsterDotNetMS.csproj
```
...
3. Run `` dotnet sonarscanner begin /d:sonar.login=admin /d:sonar.password=admin /k:"JhipsterDotNetMS" /d:sonar.host.url="http://localhost:9001" /s:"`pwd`/SonarQube.Analysis.xml" ``
...
```bash
docker build -f ./Dockerfile-Back -t jhipsterdotnetms .
```

Then run:

```bash
docker run -p 8080:80 jhipsterdotnetms
```
